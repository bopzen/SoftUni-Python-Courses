        ...
        if (line.includes(":")) {
            ...
            for (const course of courses) {
                if (course.name === courseName ) {
                    course.capacity += Number(courseCapacity);
                    exists = true;
                    break;
                }
            }
            if (!exists) {
                let newCourse = {name: courseName, capacity: Number(courseCapacity), students: []};
                courses.push(newCourse);
            }
        } else {
            let [username, credits, email, courseName] = line
                .replace("[", " | ")
                .replace("] with email"," |")
                .replace("joins", "|")
                .split(" | ");
                let student = {username, credits: Number(credits), email};
                for (const course of courses) {
                    if (course.name === courseName && course.capacity > 0) {
