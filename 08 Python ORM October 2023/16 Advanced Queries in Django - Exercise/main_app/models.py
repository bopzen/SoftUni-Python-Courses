...
class VideoGame(models.Model):
    ...
    release_year = models.PositiveIntegerField(
        validators=(
            validators.MinValueValidator(1990, "The release year must be between 1990 and 2023"),
            validators.MaxValueValidator(2023, "The release year must be between 1990 and 2023")
        )
    )
    rating = models.DecimalField(
        ...
        decimal_places=1,
        validators=(
            validators.MinValueValidator(0, "The rating must be between 0.0 and 10.0"),
            validators.MaxValueValidator(10, "The rating must be between 0.0 and 10.0")
        )
    )
...
class Task(models.Model):
    ...
    def overdue_high_priority_tasks(cls):
        query_filter = Q(priority='High') & Q(is_completed=False) & Q(completion_date__gt=F('creation_date'))
        return cls.objects.filter(query_filter)

    @classmethod
    def completed_mid_priority_tasks(cls):
        query_filter = Q(priority='Medium') & Q(is_completed=True)
        return cls.objects.filter(query_filter)
    ...
    def recent_completed_tasks(self, days):
        query_filter = Q(is_completed=True) & Q(completion_date__gte=self.creation_date-timedelta(days=days))
