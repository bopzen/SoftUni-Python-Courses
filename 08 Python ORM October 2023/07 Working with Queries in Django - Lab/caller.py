...
def order_books_by_year():
    found_books = Book.objects.all().order_by('publication_year', 'title')
    result = [
...
def delete_review_by_id(id):
    found_review = Review.objects.get(id=id)
    found_review.delete()
    return f"Review by {found_review.reviewer_name} was deleted"


def filter_authors_by_nationalities(nationality):
    found_authors = Author.objects.filter(nationality=nationality).order_by('first_name', 'last_name')
    result = [
        f"{a.biography}"
        if a.biography
...
def filter_authors_by_birth_year(first_year, second_year):
    found_authors = (Author.objects
                     .filter(birth_date__year__gte=first_year, birth_date__year__lte=second_year)
                     .order_by('-birth_date')
                     )
